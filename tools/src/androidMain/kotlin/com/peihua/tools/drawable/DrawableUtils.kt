...
package com.peihua.tools.drawable
...
import com.peihua.tools.utils.getColorInt
...
fun Any?.getStateListBgDrawable(normalColor: String?, pressedColor: String?): Drawable {
    val normalColorInt = normalColor.getColorInt(Color.WHITE)
    val pressedColorInt = pressedColor.getColorInt(Color.WHITE)
    ...
    fun getRotateDrawable(d: Drawable, angle: Float): Drawable {
        ...
        return object : LayerDrawable(arD) {
            override fun draw(canvas: Canvas) {
                canvas.withRotation(
                    angle,
                    bound.width() / 2.toFloat(),
                    bound.height() / 2.toFloat()
                ) {
                    super.draw(this)
                }
            }
...
//        stateListDrawable.addState(new int[]{-android.R.attr.state_selected}, layerDrawable2);
        ...
        val drawable =
            GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM, intArrayOf(Color.RED))
        stateListDrawable.addState(intArrayOf(R.attr.state_selected), drawable)
        stateListDrawable.addState(StateSet.WILD_CARD, Color.YELLOW.toDrawable())
